...
uint32_t expr(char *e, bool *success)
{
	...
	{
		success = false;
		if (errorCode == MATH_ERROR)Log("\33[1;31Math error");
		if (errorCode == SYNTAX_ERROR)Log("\33[1;31Syntax error");
		if (errorCode == MEMORY_ACCESS_ERROR)Log("\33[1;31Memory access error");
