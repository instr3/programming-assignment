...
#define reg_flag_set(index,res) do{if(res)cpu.eflags|=1<<(index);else cpu.eflags&=~(1<<(index));}while(0)
/*
31                  23                  15               7             0
+-------------------+---------------+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+
...
#define influence_cf(resull,sub) reg_flag_set(EFLAGS_CF,(!!((resull)&1ull<<(8*DATA_BYTE)))^(sub))
